...
public class ExtUtilsTest {
    ...
    public void run2() throws Exception {
        ...
        for (String fieldKey : keySet) {
            Object fieldVal = jsonObj.get(fieldKey);
            DanmukExart exart = getExart(stack, refId, fieldKey, fieldVal, cur_ser, cur_level, cur_level_name, Types.JSON_OBJECT);
    ...
    public static DanmukExart getExart(Deque<StackObj> stack, String refId, String fieldKey, Object fieldVal, int ser, int level, String levelName, Types outerType) {
        DanmukExart danmukExart = new DanmukExart();
        danmukExart.setId(IdWorker.getIdStr());
        danmukExart.setRefId(refId);
        danmukExart.setSpecFiled(fieldKey);
        ...
        Types type = getTypes(fieldVal);
        danmukExart.setSpecValType(type.name());
        switch (type) {
            case JSON_OBJECT:
            case JSON_ARRAY:
                StackObj stackObj = getStackObj(refId, fieldKey, fieldVal, level + 1, ser);
                stack.add(stackObj);
                break;
            default:
                danmukExart.setSpecVal(String.valueOf(fieldVal));
                break;
        }
        return danmukExart;
    }

    public static StackObj getStackObj(String refId, String fieldKey, Object fieldVal, int level, int ser) {
        StackObj item = new StackObj();
        item.setRefId(refId);
        item.setSer(ser);
        item.setCurLevel(level);
        item.setCurLevelName(fieldKey);
        item.setOuterFieldKey(fieldKey);
        if (fieldVal instanceof JSONObject) {
            item.setJsonObj((JSONObject) fieldVal);
        } else if (fieldVal instanceof JSONArray) {
            item.setJsonAry((JSONArray) fieldVal);
        }
        return item;
    }
    ...
    public static void processChild(List<DanmukExart> danmukExarts, Deque<StackObj> stack) {
        while (!stack.isEmpty()) {
            ...
            String outer_field_key = child.getOuterFieldKey();
            ...
            switch (outerType) {
                case JSON_OBJECT:
                    ...
                    for (String fieldKey : keySet) {
                        Object fieldVal = jsonObj.get(fieldKey);
                        DanmukExart exart = getExart(stack, refId, fieldKey, fieldVal, cur_ser, cur_level, cur_level_name, outerType);
                        danmukExarts.add(exart);
                        cur_ser++;
                    }
                    break;
                case JSON_ARRAY:
                    ...
                    for (int i = 0; i < jsonAry.size(); i++) {
                        Object fieldVal = jsonAry.get(i);
                        DanmukExart exart = getExart(stack, refId, outer_field_key, fieldVal, cur_ser, cur_level, cur_level_name, outerType);
                        danmukExarts.add(exart);
                        cur_ser++;
                    }
                    break;
